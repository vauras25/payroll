...
$(document).ready(function() {
    ...
    if ($('.sideleft-scrollbar').length > 0) {
    ...
    $(document).on('keydown', function(event) {
        if (event.key == "Escape") {
            ...
            let activemodals = $('.modal.show');
            if (activemodals.length > 1) {
                const element = activemodals.slice(-1);
                $(element).modal('hide');
            } else {
                $('.modal').modal('hide');
            }
    ...
    $(document).click(function(event) {
        ...
        if (!$target.hasClass('skip-popup') && !$target.closest('.bd_rightside').length && $('.bd_rightside').is(":visible") && !$target.closest('button').length) {
            let flag = true;

            let parents = $target.parents();
            if (parents.length > 0) {
                if ($(parents[0]).hasClass('available-items')) {
                    flag = false;
                }

                if ($(parents[0]).find('span.nsdicon-close').length) {
                    flag = false;
                }
            }
