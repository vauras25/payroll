...
export class TdsComponent implements OnInit {
  ...
  initChilds(items: any = [],deductionItems:any=[])
    {
      ...
      if(element?.child.length>0)
      {
        element?.child.forEach((subElem:any) => {
          ...
          if(eighty_c)
          {
            if(eighty_c.child && eighty_c.child.length>0)
              {
  ...
  add(event:any)
  {
    ...
    this.commonService.postFormDataNested("employee/employee-declaration-modify",submitData).subscribe((res:any) => {
      this.spinner.hide();    
    if (res.status == 'success') {
  ...
  {
    this.spinner.show();
    this.commonService.postData("employee/employee-declaration",{
      type:"temp"
    }).subscribe(res => {
      this.spinner.hide();
      if (res.status == 'success') {
