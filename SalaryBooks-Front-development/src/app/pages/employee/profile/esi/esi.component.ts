...
export class EsiComponent implements OnInit {
  ...
  prev_employment:boolean=true;
  @Output() submitESI = new EventEmitter<any>();
...
updateEmployeeEmploymentDetails(event: any) {
    ...
    if (this.employmentForm.valid) {
        ...
        if (this.prev_employment) {
            payload.pre_er_pf = "yes";

            payload.pre_er_details = ({
        ...
        payload.esic_family_details = (esic_family_details);
        payload.pf_nominee_details = (pf_nominee_details);

        this.commonService.postDataRaw("employee/update-employee-pf-esic-details",payload).subscribe(res => {
            if (res.status == 'success') {
                this.toastr.success(res.message);
                this.cancelEntry();
                this.submitESI.emit(true);
...
fetchEmployeeDetails() {
  ...
  if (this.employee_details?.employee_details?.pf_esic_details?.pre_er_pf == 'yes') {
      this.prev_employment=true;
  } else {
      $('#previousemployment-toggle')?.removeClass('on');
      this.prev_employment=false;

  }
  this.employmentForm.patchValue({
      'pre_er_details': {
          'er_name': this.employee_details?.employee_details?.pf_esic_details?.pre_er_details?.er_name ?? DEFAULTVAL,
          'last_drawn_gross': this.employee_details?.employee_details?.pf_esic_details?.pre_er_details?.last_drawn_gross ?? DEFAULTVAL,
          'contact_no': this.employee_details?.employee_details?.pf_esic_details?.pre_er_details?.contact_no ?? DEFAULTVAL,
          'reporting_to': this.employee_details?.employee_details?.pf_esic_details?.pre_er_details?.reporting_to ?? DEFAULTVAL,
          'exit_date': this.employee_details?.employee_details?.pf_esic_details?.pre_er_details?.exit_date ?? DEFAULTVAL,
          'last_designation': this.employee_details?.employee_details?.pf_esic_details?.pre_er_details?.last_designation ?? DEFAULTVAL,
      },
      'pre_er_epfo_details': {
          'uan_no': this.employee_details?.employee_details?.pf_esic_details?.pre_er_epfo_details?.uan_no ?? DEFAULTVAL,
          'last_member_id': this.employee_details?.employee_details?.pf_esic_details?.pre_er_epfo_details?.last_member_id ?? DEFAULTVAL,
          'last_ro': this.employee_details?.employee_details?.pf_esic_details?.pre_er_epfo_details?.last_ro ?? DEFAULTVAL,
      },
      'pre_er_esic_details': {
          'esic_no': this.employee_details?.employee_details?.pf_esic_details?.pre_er_esic_details?.esic_no ?? DEFAULTVAL,
          'ip_dispensary': this.employee_details?.employee_details?.pf_esic_details?.pre_er_esic_details?.ip_dispensary ?? DEFAULTVAL,
          'family_dispensary': this.employee_details?.employee_details?.pf_esic_details?.pre_er_esic_details?.family_dispensary ?? DEFAULTVAL,
      },
      'curr_er_epfo_details': {
          'uan_no': this.employee_details?.employee_details?.pf_esic_details?.curr_er_epfo_details?.uan_no ?? DEFAULTVAL,
          'last_member_id': this.employee_details?.employee_details?.pf_esic_details?.curr_er_epfo_details?.last_member_id ?? DEFAULTVAL,
          'last_ro': this.employee_details?.employee_details?.pf_esic_details?.curr_er_epfo_details?.last_ro ?? DEFAULTVAL,
          'membership_date': this.employee_details?.employee_details?.pf_esic_details?.curr_er_epfo_details?.membership_date ?? DEFAULTVAL,
      },
      'curr_er_esic_details': {
          'esic_no': this.employee_details?.employee_details?.pf_esic_details?.curr_er_esic_details?.esic_no ?? DEFAULTVAL,
          'ip_dispensary': this.employee_details?.employee_details?.pf_esic_details?.curr_er_esic_details?.ip_dispensary ?? DEFAULTVAL,
          'family_dispensary': this.employee_details?.employee_details?.pf_esic_details?.curr_er_esic_details?.family_dispensary ?? DEFAULTVAL,
          'membership_date': this.employee_details?.employee_details?.pf_esic_details?.curr_er_esic_details?.membership_date ?? DEFAULTVAL,
      }
  });
  (this.employmentForm.controls['esic_family_details'] as UntypedFormArray).clear();
  (this.employmentForm.controls['pf_nominee_details'] as UntypedFormArray).clear();

  (this.employee_details?.employee_details?.pf_esic_details?.esic_family_details ?? []).forEach((details: any) => {
      this.addFormRows(this.employmentForm, 'esic_family_details', details)
  });

  (this.employee_details?.employee_details?.pf_esic_details?.pf_nominee_details ?? []).forEach((details: any) => {
...
shuffleFields() {
    ...
    //     }
    // }, 100);
    this.prev_employment=!this.prev_employment;
}
