...
@Component({
  selector: 'admin-app-bonus-policy',
  templateUrl: './bonus-policy.component.html',
  styleUrls: ['./bonus-policy.component.css']
})
export class ADBonusPolicyComponent implements OnInit {
  ...
  fetch() {
    this.dtOptions = {
      ajax: (dataTablesParameters: any, callback) => {
        this.adminService.fetchBonusPolicies({
  ...
  changePublishStatus(item: any) {
    this.adminService.updateBonusPolicyPublishStatus({
  ...
  getEdit(item: any) {
    ...
    this.initialValueBeforeUpdate = {
      ...
      'publish_status': this.bonusPolicyForm.value.publish_status.value,
    }
  }
  ...
  create(event: any) {
    if (this.bonusPolicyForm.valid) {
      ...
      this.adminService.createBonusPolicy({
  ...
  deleteItem(item: any) {
    swal.fire({
      ...
      if (result.value) {
        this.adminService.deleteBonusPolicy({
          'bonus_policy_id': item._id,
        }).subscribe(res => {
          if (res.status == 'success') {
            this.toastr.success(res.message);
  ...
  update(event: any) {
    if (this.bonusPolicyForm.valid) {
      ...
      this.adminService.updateBonusPolicy(
        documentUpdate
      ).subscribe(res => {
        if (res.status == 'success') {
          this.toastr.success(res.message);
