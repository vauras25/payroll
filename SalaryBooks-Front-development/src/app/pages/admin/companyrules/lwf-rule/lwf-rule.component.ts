...
@Component({
  selector: 'admin-app-lwf-rule',
  templateUrl: './lwf-rule.component.html',
  styleUrls: ['./lwf-rule.component.css']
})
export class ADLwfRuleComponent implements OnInit {
  ...
  //       return this.formBuilder.group({
  //         from_date: [data.from_date ?? null, Validators.compose([Validators.required])],
  //         to_date: [data.to_date ?? null, Validators.compose([Validators.required])],
  //         wage_band: [data.wage_band ?? null, Validators.compose([])],
  //         wage_from: [data.wage_from ?? null, Validators.compose([])],
  //         wage_to: [data.wage_to ?? null, Validators.compose([])],
  //         employee_contribution: [data.employee_contribution ?? null, Validators.compose([Validators.required])],
  //         employer_contribution: [data.employer_contribution ?? null, Validators.compose([Validators.required])],
  //         gross_type: [data.gross_type ?? null, Validators.compose([Validators.required])],
  ...
  fetchStateMaster() {
    ...
    return new Promise((resolve, reject) => {
      _this.spinner.show();
      _this.adminService.fetchStates()
        .subscribe(res => {
          if (res.status == "success") {
            ...
            for (const key in res.state_list[0].states) {
              if (Object.prototype.hasOwnProperty.call(res.state_list[0].states, key)) {
                ...
                _this.stateMaster.push({ "state_code": element.state_code, "description": element.name });
              }
            }
          } else {
  ...
  add(event: any) {
    ...
    if (this.lwfRuleForm.valid && template_data) {
      ...
      this.adminService.createLwfRule(form_data).subscribe(
        (res) => {
          if (res.status == 'success') {
  ...
  edit(event: any) {
    ...
    if (this.lwfRuleForm.valid && template_data) {
      ...
      this.adminService.updateLwfRule(form_data).subscribe(
        (res) => {
          if (res.status == 'success') {
            this.toastr.success(res.message);
            this.fetch();
  ...
  fetch(page: any = null) {
    ...
    this.spinner.show();
    this.adminService.fetchLwfRules({
  ...
  deleteItem(item: any) {
    swal.fire({
      ...
      if (result.value) {
        this.adminService.deleteLwfRule({
  ...
  changePublishStatus(item: any) {
    this.adminService.updateLwfRulePublishStatus({
      'lwf_rule_id': item._id,
