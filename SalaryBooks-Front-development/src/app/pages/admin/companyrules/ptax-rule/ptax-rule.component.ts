...
@Component({
  selector: 'admin-app-ptax-rule',
  templateUrl: './ptax-rule.component.html',
  styleUrls: ['./ptax-rule.component.css']
})
export class ADPtaxRuleComponent implements OnInit {
  ...
  constructor(
    ...
    ngOnInit() {
      ...
      //   // let index = this.pTaxRuleForm.get("tax_range_amount")?.value?.length - 1
      //   // if(index >= 0){
  ...
  // initTemplateRows(type: any, amount_from: any = null, amount_to: any = null, tax_amount: any = null) {
  //   switch (type) {
 ...
 initNestedTemplateRows(
  ...
  // addTemplateRows(type: any, amount_from: any = null, amount_to: any = null, tax_amount: any = null) {
  //   const control = <UntypedFormArray>this.pTaxRuleForm.get(type);
  //   control.push(this.initTemplateRows(type, amount_from, amount_to, tax_amount));
  // }
  ...
  addTemplateRows(
    ...
    // if (i >= 0) {
      ...
      // console.log(lastControl.get('amount_to'));
    // }
    // }
    if(last_slab == 'yes'){
      last_slab = true
  ...
  fetch() {
    this.dtOptions = {
      ajax: (dataTablesParameters: any, callback) => {
        this.adminService.fetchPTaxRules({
      ...
      columns: [
        ...
        {
          render: function (data, type, full, meta) {
            return (full.salary_type) ? `<span class="text-capitalize">` + full.salary_type + `</span>` : 'N/A';
          },
          orderable: true,
          name: 'salary_type'
        },
        {
          render: function (data, type, full, meta) {
            return (full.settlement_frequency) ? `<span class="text-capitalize">` + full.settlement_frequency + `</span>` : 'N/A';
          },
          orderable: true,
  ...
  changePublishStatus(item: any) {
    this.adminService.updatePTaxPublishStatus({
  ...
  async create(event: any) {
    if (this.pTaxRuleForm.valid) {
      ...
      this.adminService.createPTaxRule({
        // state_id: this.pTaxRuleForm.value.state.id,
        state_name: this.pTaxRuleForm.value.state?.description ?? "",
        template_name: this.pTaxRuleForm.value.template_name,
        // state_name: this.pTaxRuleForm.value.state,
        effective_from: this.datePipe.transform(this.pTaxRuleForm.value.effective_from, 'Y-MM-dd'),
        salary_type: this.pTaxRuleForm.value.salary_type.value,
        settlement_frequency: this.pTaxRuleForm.value.settlement_frequency.value,
        tax_range_amount: JSON.stringify(this.pTaxRuleForm.value.tax_range_amount),
        periods: JSON.stringify(periods),
        publish_status: this.pTaxRuleForm.value.publish_status.value,
      }).subscribe(res => {
        if (res.status == 'success') {
          this.toastr.success(res.message);
          // this.pTaxRuleForm.reset();
  ...
  deleteItem(item: any) {
    swal.fire({
      ...
      if (result.value) {
        this.adminService.deletePTaxRule({
          'ptax_rule_id': item._id,
        }).subscribe(res => {
          if (res.status == 'success') {
            this.toastr.success(res.message);
  ...
  async update(event: any) {
    if (this.pTaxRuleForm.valid) {
      ...
      this.adminService.updatePTaxRule(
        documentUpdate
      ).subscribe(res => {
        if (res.status == 'success') {
          this.toastr.success(res.message);
  ...
  getUpdateHistory() {
    if (this.viewPTaxTemplate != null && this.viewPTaxTemplate.history != null && Array.isArray(this.viewPTaxTemplate.history)) {
  ...
  configLastSlab(t: any) {
    ...
    if(i >= 0){
      const contol:AbstractControl = this.getTemplateRows('tax_range_amount')[i];
      if (t.checked) {
