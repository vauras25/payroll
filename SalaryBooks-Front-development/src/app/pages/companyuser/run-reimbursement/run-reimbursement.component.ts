...
@Component({
  selector: 'app-run-reimbursement',
  templateUrl: './run-reimbursement.component.html',
  styleUrls: ['./run-reimbursement.component.css']
})
export class RunReimbursementComponent implements OnInit {
  Global = Global;
  ...
  fetchEmployees({
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (loading == true) this.spinner.show(payload);
  ...
  runReimbursement()
  {
    ...
    payload={
      ...
      "checked_row_ids": JSON.stringify(this.checkedRowIds),
      "unchecked_row_ids": JSON.stringify(this.uncheckedRowIds),
      "attendance_month": payload?.wage_month,
      "attendance_year": payload?.wage_year
  };
    return new Promise((resolve, reject) => {
      this.spinner.show();
      
      this.companyuserService.runReimbursement(payload)
        .subscribe((res: any) => {
          this.spinner.hide();
          if (res.status == 'success') {
            this.toastr.success(res.message);
