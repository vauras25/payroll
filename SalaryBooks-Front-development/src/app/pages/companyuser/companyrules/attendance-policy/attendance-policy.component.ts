...
@Component({
  selector: 'companyuser-app-attendance-policy',
  templateUrl: './attendance-policy.component.html',
  styleUrls: ['./attendance-policy.component.css']
})
export class CMPAttendancePolicyComponent implements OnInit {
  ...
  fetch() {
    this.dtOptions = {
      ajax: (dataTablesParameters: any, callback) => {
        this.companyuserService.fetchAttendancePolicies({
  ...
  changePublishStatus(item: any) {
    this.companyuserService.updateAttendancePublishStatus({
  ...
  changeStatus(item: any) {
    this.companyuserService.updateAttendanceStatus({
      'attedance_id': item._id,
  ...
  getEdit(item: any) {
    ...
    this.initialValueBeforeUpdate = {
      ...
      'publish_status': this.attendancePolicyForm.value.publish_status?.value ?? "published",
    }
  ...
  create(event: any) {
    ...
    if (this.attendancePolicyForm.valid) {
      ...
      this.companyuserService.createAttendancePolicy({
  ...
  deleteItem(item: any) {
    swal.fire({
      ...
      if (result.value) {
        this.companyuserService.deleteAttendancePolicy({
          'attedance_id': item._id,
        }).subscribe(res => {
          if (res.status == 'success') {
            this.toastr.success(res.message);
  ...
  update(event: any) {
    ...
    if (this.attendancePolicyForm.valid) {
      ...
      this.companyuserService.updateAttendancePolicy(
        documentUpdate
      ).subscribe(res => {
        if (res.status == 'success') {
          this.toastr.success(res.message);
  ...
  fetchTemplateLibrary() {
    ...
    this.dtOptionsLibrary = {
      ajax: (dataTablesParameters: any, callback) => {
        this.companyuserService.fetchAttendancePolicyLibrary({
          'pageno': Math.floor((dataTablesParameters.start + dataTablesParameters.length) / dataTablesParameters.length), 'perpage': dataTablesParameters.length,
          'searchkey': dataTablesParameters.search.value,
          'sortbyfield': Global.getTableSortingOptions('sortbyfield', dataTablesParameters),
          'ascdesc': Global.getTableSortingOptions('ascdesc', dataTablesParameters)
          ...
          if (res.status == 'success') {
            callback({
              recordsTotal: res.attendance_rule.totalDocs,
              recordsFiltered: res.attendance_rule.totalDocs,
              data: res.attendance_rule.docs,
            });
          } else {
            this.toastr.error(res.message);
