...
@Component({
  selector: 'app-companyuser-master-sheet-report',
  templateUrl: './master-sheet.component.html',
  styleUrls: ['./master-sheet.component.css'],
})
export class CMPMasterSheetReportComponent implements OnInit, OnDestroy, OnChanges {
  Global = Global;
  @Input() sheetTemplate: any;
  @Input() empReportData: any;
  ...
  getSheetTemplateHeadColspan(
    ...
    let _sTemplate: any[] = this.sheetTemplate;
  ...
  restoreReportTempData() {
    ...
    let attendance_head_includes: any[] = [];
    let head_includes: any[] = [];
    let extra_earning_data: any[] = [];

    if (this.empReportData.length > 0) {
      // Generating Report Available Heads
      this.empReportData.forEach((reportData: any, i:number) => {
        (reportData.master_report?.heads ?? []).forEach((head: any) => {
          let exist =
      ...
      // Generating Attendance Report Heads
      this.empReportData.forEach((reportData: any) => {
          (reportData.attendance_heads ?? []).forEach((head: any) => {
              let exist =
                attendance_head_includes.find((obj: any) => {
                  return obj.leave_temp_head_id == head.leave_temp_head_id;
                }) ?? null;

              if (!exist) {
                attendance_head_includes.push(head);
              }
          });
      });
      ...
      //     let exist =
      //     attendance_head_includes.find((obj: any) => {
      //         return obj.leave_type_name == head.leave_type_name;
      //       }) ?? null;

      //     if (!exist) {
      //       attendance_head_includes.push({
      //         amount: head.amount,
      //         head_id: head.leave_temp_head_id,
      //         leave_type_name: head.leave_type_name,
      //         head_rate: head.available,
      //         head_rate_type: head.head_rate_type,
      //         head_title: head.head_title,
      //         head_abbreviation: head.abbreviation,
      //       });
      //     }
      //   });
      // });
      ...
      // Generating Report Available Heads
      this.empReportData.forEach((reportData: any) => {
        (reportData.salary_report?.heads ?? []).forEach((head: any) => {
          let exist =
    ...
    this.empReportTempData = {
      ...
      attendance_head_includes: attendance_head_includes,
  ...
  getHeadValue(report:any, headType:string, heads: any[], head_id: any, key: any, i:number) {
    let head =
    heads.find((obj: any) => {
    ...
    if (head) {
    let retVal = report[`${headType}_report`]?.heads?.find((d:any) =>  d.head_id == head.head_id)
    if(retVal?.[key] && typeof retVal?.[key] !== 'string'){
      retVal[key] = retVal?.[key]?.toFixed(2)
    }
    return retVal?.[key] ?? 0;
  } else {
    return 'N/A';
  ...
  getHeadValue2(report:any, head: any) {

    if (head) {
      let retVal = report.attendance_heads?.find((d:any) =>  d.leave_temp_head_id == head.leave_temp_head_id)
  
      return retVal?.available ?? 0;
    } else {
      return 'N/A';
    }
  }
  ...
  getExtraEarningValue(
    ...
    if (earning) {
      if(earning?.[key] && typeof earning?.[key] !== 'string'){                       
        earning[key] = earning?.[key]?.toFixed(2)
      }
    ...
    if (module) {
      ...
      if (field) {
        ...
        if(Array.isArray(field.mapping)) {
          let mainValue:any = 0
          field.mapping.forEach((field:any) => {
...
// export class CMPMasterSheetReportComponent implements OnInit, OnChanges {
...
//     }
//     ngOnChanges(){
//       this._tempSheetTemplate = this.selectedTemplate
//     }
