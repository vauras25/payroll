...
@Component({
  selector: 'app-reimbursement-bank',
  templateUrl: './reimbursement-bank.component.html',
  styleUrls: ['./reimbursement-bank.component.css']
})
export class ReimbursementBankComponent implements OnInit {
  Global = Global;
          ...
          var docs: any[] = res?.employees?.docs ?? [];
           docs.forEach((doc: any) => {
              ...
              doc.employee_monthly_reports.forEach((element:any) => {
                ...
                element?.reimbursment_report?.heads.forEach((Sub_element:any) => {
                  reimbursment_amnt+=  Sub_element.amount;
                });  
              });
              doc.reimbursment_amnt=reimbursment_amnt;
 ...
 async excelExport()
  ...
  getSheetGenerationStatus(emp_details: any): { status: string, label: string, checkbox: boolean } {
    if (
        emp_details?.employee_monthly_reports?.reimbursment_report &&
        emp_details?.employee_monthly_reports?.reimbursment_report?.bank_ins_referance_id &&
        emp_details?.employee_monthly_reports?.reimbursment_report?.bank_instruction_status === 'confirm'
    ) {
        return { 'status': "confirm", 'label':  emp_details?.employee_monthly_reports?.reimbursment_report?.bank_ins_referance_id, 'checkbox': false };
    } 
...
fetchBankData({
  ...
  return new Promise(async (resolve, reject) => {
    ...
    payload.checked_row_ids = "[]";
    payload.unchecked_row_ids = "[]";
    payload.wage_month=this.filterForm.value?.wage_month?.index;
    payload.wage_year=this.filterForm.value?.wage_year;
    ...
    if (loading == true) this.spinner.show();
    this.companyuserService.getReimbursement(payload).subscribe((res: any) => {
      if (res.status == 'success') {
