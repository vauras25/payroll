...
@Component({
  selector: 'app-extra-deduction',
  templateUrl: './extra-deduction.component.html',
  styleUrls: ['./extra-deduction.component.css']
})
export class ExtraDeductionComponent implements OnInit {
  Global = Global;
  ...
  fetchEmployees({
    ...
    return new Promise(async (resolve, reject) => {
      ...
      this.companyuserService.pendingEmployeeExtraEarnings(payload).subscribe((res: any) => {
        if (res.status == 'success') {
  ...
  bulkApprove(ev:any)
  {
    return new Promise((resolve, reject) => {
      ...
      let payload={row_checked_all:this.rowCheckedAll.toString(),checked_row_ids:this.checkedRowIds,
        unchecked_row_ids:this.uncheckedRowIds,status:this.approveForm.value.status,remark:this.approveForm.value.note
        ,wage_month: this.getReportPayload().wage_month,wage_year:this.getReportPayload().wage_year
