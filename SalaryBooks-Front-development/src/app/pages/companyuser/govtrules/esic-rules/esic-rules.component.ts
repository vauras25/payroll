...
@Component({
  selector: 'companyuser-app-esic-rules',
  templateUrl: './esic-rules.component.html',
  styleUrls: ['./esic-rules.component.css'],
})
export class CMPEsicRulesComponent implements OnInit {
  ...
  constructor(
    ...
    if (
      !Global.checkCompanyModulePermission({
        ...
        company_operation: 'gov_esic_rule',
        company_sub_module: ['esi_rules'],
  ...
  ngOnInit() {
    ...
    if (
      !Global.checkCompanyModulePermission({
        company_module: 'gov_esic_rule',
        company_operation: ['customizable', 'default'],
        // staff_module: 'gov_esic_rule',
  ...
  // console.log(this.preferenceGroup);
  }
      ...
      columns: [
        ...
        },
        {
          render: function (data, type, full, meta) {
            ...
            return (
              `Period A ` +
              month[full.contribution_period_a_from - 1] +
              ` to ` +
              month[full.contribution_period_a_to - 1] +
              ` <br>Period B ` +
              month[full.contribution_period_b_from - 1] +
              ` to ` +
              month[full.contribution_period_b_to - 1]
            );
          },
          orderable: false,
        },
  ...
  getEdit(item: any) {
    ...
    this.initialValueBeforeUpdate = {
      ...
      employee_contribution: this.esicRuleForm.value.employee_contribution
        ...
        .trim(),
      effective_date: this.datePipe.transform(
        this.esicRuleForm.value.effective_date,
        'Y-MM-dd'
      ),
      contribution_period_a_from:
        this.esicRuleForm.value.contribution_period_a_from.id,
      contribution_period_a_to:
        this.esicRuleForm.value.contribution_period_a_to.id,
      contribution_period_b_from:
        this.esicRuleForm.value.contribution_period_b_from.id,
      contribution_period_b_to:
        this.esicRuleForm.value.contribution_period_b_to.id,
    };
  }
  ...
  cancelEdit() {
    ...
    for (const key in this.esicRuleForm.controls) {
      if (
        Object.prototype.hasOwnProperty.call(this.esicRuleForm.controls, key)
      ) {
  ...
  update(event: any) {
    if (this.esicRuleForm.valid) {
      ...
      this.companyuserService.updateEsicRule(documentUpdate).subscribe(
        (res) => {
          if (res.status == 'success') {
            this.toastr.success(res.message);
  ...
  copyDefaultTemplate() {
    ...
    this.companyuserService.getActiveEsicRule().subscribe(
      (res: any) => {
        if (res.status == 'success') {
          if (res.esic && Object.keys(res.esic).length !== 0) {
            let contribution_period_a_from = this.monthMaster.find(
              (obj: any) => {
                return obj.id == res.esic?.contribution_period_a_from;
              }
            );

            let contribution_period_a_to = this.monthMaster.find((obj: any) => {
              return obj.id == res.esic?.contribution_period_a_to;
            });

            let contribution_period_b_from = this.monthMaster.find(
              (obj: any) => {
                return obj.id == res.esic?.contribution_period_b_from;
              }
            );

            let contribution_period_b_to = this.monthMaster.find((obj: any) => {
              return obj.id == res.esic?.contribution_period_b_to;
            });

            this.esicRuleForm.patchValue({
              circular_no: res.esic?.circular_no,
              wage_ceiling: res.esic?.wage_ceiling,
              round_off: res.esic?.round_off,
              employer_contribution: res.esic?.employer_contribution,
              employee_contribution: res.esic?.employee_contribution,
              effective_date: this.datePipe.transform(
                res.esic?.effective_date,
                'MM/dd/YYYY'
              ),
              contribution_period_a_from: contribution_period_a_from,
              contribution_period_a_to: contribution_period_a_to,
