...
@Component({
  selector: 'companyuser-app-gratuity-rules',
  templateUrl: './gratuity-rules.component.html',
  styleUrls: ['./gratuity-rules.component.css'],
})
export class CMPGratuityRulesComponent implements OnInit {
  ...
  constructor(
    ...
    //   company_operation:"gov_gratuity_rule",
    //   company_sub_module: ['gratuity'],
    ...
    this.gratuityRuleForm = formBuilder.group({
      ...
      service_year_no_applicable: [
        null,
        Validators.compose([Validators.required]),
      ],
      fifth_year_atrendance_rule: [
        null,
        Validators.compose([Validators.required]),
      ],
      fifth_year_atrendance: [
        null,
        Validators.compose([
          Validators.required,
          Validators.pattern('^[0-9]*$'),
      ...
      fifth_year_atrendance_applicable: [
        null,
        Validators.compose([Validators.required]),
      ],
      max_gratuity_anual_rule: [
        null,
        Validators.compose([Validators.required]),
      ],
      max_gratuity_anual: [
  ...
  ngOnInit() {
    ...
    if (
      !Global.checkCompanyModulePermission({
        company_module: 'gov_gratuity_rule',
    ...
    if (
      ...
      Global.checkCompanyModulePermission({
        ...
        staff_module: 'gov_gratuity_rule',
        staff_operation: ['view'],
      }) ||
      Global.checkCompanyModulePermission({
        company_module: 'government_rules',
        company_sub_module: ['gratuity'],
  ...
  fetch() {
    this.dtOptions = {
      ...
      columns: [
          ...
          render: function (data, type, full, meta) {
            ...
            switch (full.employee_no_rule) {
              case 'greaterthanequal':
                html += `<i class="fas fa-greater-than-equal"></i>&nbsp;`;
                break;
              case 'greater':
                html += `<i class="fas fa-greater-than"></i>&nbsp;`;
                break;
              case 'less':
                html += `<i class="fas fa-less-than"></i>&nbsp;`;
                break;
              case 'lessthanequal':
                html += `<i class="fas fa-less-than-equal"></i>&nbsp;`;
                break;
              case 'equal':
                html += `<i class="fas fa-less-than"></i>&nbsp;`;
                break;
        ...
        {
          render: function (data, type, full, meta) {
            return full.income_tax_applicable == 'yes'
              ? 'Applicable'
              : 'Not Applicable';
          },
          orderable: true,
  ...
  getEdit(item: any) {
    ...
    this.initialValueBeforeUpdate = {
      ...
      employee_no_applicable:
        this.gratuityRuleForm.value.employee_no_applicable.id,
      ...
      service_year_no: this.gratuityRuleForm.value.service_year_no
        ...
        .trim(),
      service_year_no_applicable:
        this.gratuityRuleForm.value.service_year_no_applicable.id,
      fifth_year_atrendance_rule:
        this.gratuityRuleForm.value.fifth_year_atrendance_rule.id,
      fifth_year_atrendance: this.gratuityRuleForm.value.fifth_year_atrendance
        .toString()
        .trim(),
      fifth_year_atrendance_applicable:
        this.gratuityRuleForm.value.fifth_year_atrendance_applicable.id,
      max_gratuity_anual_rule:
        this.gratuityRuleForm.value.max_gratuity_anual_rule.id,
      max_gratuity_anual: this.gratuityRuleForm.value.max_gratuity_anual
        .toString()
        .trim(),
      max_gratuity_anual_applicable:
        this.gratuityRuleForm.value.max_gratuity_anual_applicable.id,
      income_tax_applicable:
        this.gratuityRuleForm.value.income_tax_applicable.id,
      effective_date: this.datePipe.transform(
        this.gratuityRuleForm.value.effective_date,
        'Y-MM-dd'
      ),
    };
    ...
    for (const key in this.gratuityRuleForm.controls) {
      if (
        Object.prototype.hasOwnProperty.call(
          this.gratuityRuleForm.controls,
          key
        )
      ) {
      ...
      this.companyuserService
        .createGratuityRule({
          ...
          service_year_no_rule:
            this.gratuityRuleForm.value.service_year_no_rule.id,
          service_year_no: this.gratuityRuleForm.value.service_year_no,
          service_year_no_applicable:
            this.gratuityRuleForm.value.service_year_no_applicable.id,
          fifth_year_atrendance_rule:
            this.gratuityRuleForm.value.fifth_year_atrendance_rule.id,
          fifth_year_atrendance:
            this.gratuityRuleForm.value.fifth_year_atrendance,
          fifth_year_atrendance_applicable:
            this.gratuityRuleForm.value.fifth_year_atrendance_applicable.id,
          max_gratuity_anual_rule:
            this.gratuityRuleForm.value.max_gratuity_anual_rule.id,
  ...
  update(event: any) {
    if (this.gratuityRuleForm.valid) {
      ...
      this.companyuserService.updateGratuityRule(documentUpdate).subscribe(
        (res) => {
          if (res.status == 'success') {
            this.toastr.success(res.message);
  ...
  copyDefaultTemplate() {
    ...
    this.companyuserService.getActiveGratuityRule().subscribe(
      (res: any) => {
        if (res.status == 'success') {
          if (res.gratuity && Object.keys(res.gratuity).length !== 0) {
            this.gratuityRuleForm.patchValue({
              employee_no: res.gratuity?.employee_no,
              service_year_no: res.gratuity?.service_year_no,
              fifth_year_atrendance: res.gratuity?.fifth_year_atrendance,
              max_gratuity_anual: res.gratuity?.max_gratuity_anual,

              employee_no_rule: this.rulesMaster.find((obj) => {
                return obj.id === res.gratuity?.employee_no_rule;
              }),

              employee_no_applicable: this.applicableMaster.find((obj) => {
                return obj.id === res.gratuity?.employee_no_applicable;
              }),

              service_year_no_rule: this.rulesMaster.find((obj) => {
                return obj.id === res.gratuity?.service_year_no_rule;
              }),

              service_year_no_applicable: this.applicableMaster.find((obj) => {
                return obj.id === res.gratuity?.service_year_no_applicable;
              }),

              fifth_year_atrendance_rule: this.rulesMaster.find((obj) => {
                return obj.id === res.gratuity?.fifth_year_atrendance_rule;
              }),

              fifth_year_atrendance_applicable: this.applicableMaster.find(
                (obj) => {
                  return (
                    obj.id === res.gratuity?.fifth_year_atrendance_applicable
                  );
                }
              ),

              max_gratuity_anual_rule: this.rulesMaster.find((obj) => {
                return obj.id === res.gratuity?.max_gratuity_anual_rule;
              }),

              max_gratuity_anual_applicable: this.applicableMaster.find(
                (obj) => {
                  return obj.id === res.gratuity?.max_gratuity_anual_applicable;
                }
              ),

              income_tax_applicable: this.applicableMaster.find((obj) => {
                return obj.id === res.gratuity?.income_tax_applicable;
              }),

              effective_date: this.datePipe.transform(
                res.gratuity?.effective_date,
                'MM/dd/YYYY'
              ),
            });
          } else {
