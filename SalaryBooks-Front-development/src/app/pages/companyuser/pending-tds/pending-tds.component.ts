...
@Component({
  selector: 'app-pending-tds',
  templateUrl: './pending-tds.component.html',
  styleUrls: ['./pending-tds.component.css']
})
export class PendingTdsComponent implements OnInit {
  Global = Global;
  ...
  getReportPayload() {
    let payload: any = {
      ...
      status:this.employeeFilter?.approval_status?.value ?? '',
    };
    return payload;
  }
  ...
  fetchEmployees({
    ...
    return new Promise(async (resolve, reject) => {
      ...
      payload.type="reimbursement";

      let wage_date_from:any=new Date(this.getReportPayload().wage_year_from,this.getReportPayload().wage_month_from, 1);
      let wage_date_to:any=new Date(this.getReportPayload().wage_year_to,(this.getReportPayload().wage_month_to+1), 0);

      wage_date_from=moment(wage_date_from).format('YYYY-MM-DD');
      wage_date_to=moment(wage_date_to).format('YYYY-MM-DD');
      payload.wage_date_from=wage_date_from;
      payload.wage_date_to=wage_date_to;

      if (loading == true) this.spinner.show();
      this.companyuserService.getpendingTDS(payload).subscribe((res: any) => {
        if (res.status == 'success') {
  ...
  bulkApprove(ev:any)
  {
    return new Promise((resolve, reject) => {
      ...
      let payload={row_checked_all:this.rowCheckedAll.toString(),checked_row_ids:this.checkedRowIds,
        ...
        ,wage_date_from: wage_date_from,wage_date_to:wage_date_to
  ...
  rediretDetail(item:any)
  {
    this.router.navigateByUrl(`company/approval/tds-view/${item?.employee_id}`);
  }
}
