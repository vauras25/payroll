...
var storage =   multer.diskStorage({
    destination: function (req, file, callback) {
      callback(null, '../../storage/admin/company_logo');
    },
...
module.exports = {
    get_bonus_slip_library: async function (req, resp, next) {
        try{
            ...
            if (!matched) {
                ...
                BonusSlipTemp.paginate(filter_option,options,  function (err, bonustemp_rule) {
                    if (err) return resp.json({ status: 'error', message: err.message });
                    return resp.status(200).json({ status: 'success', bonustemp_rule: bonustemp_rule });
                })
            }
    ...
    get_bonus_slip_temp: async function (req, resp, next) {
        try{
            const v = new Validator(req.body, {
                pageno: 'required',
            });
            ...
            if (!matched) {
                ...
                //console.log(filter_option);
                BonusSlipTemp.paginate(filter_option,options,  function (err, bonus_slip_temp) {
                    if (err) return resp.json({ status: 'error', message: err.message });
                    return resp.status(200).json({ status: 'success', bonus_slip_temp: bonus_slip_temp });
                })
            }
        }
        catch (e) {
    ...
    add_bonus_slip_temp: async function (req, resp) {
        try{
            const v = new Validator(req.body, {
            ...
            if (!matched) {
                ...
                var document = {
                    ...
                    bonus_details           : JSON.parse(req.body.bonus_details),
                ...
                if(req.body.logo_status == 'changed')
                {
                    ...
                    if(file.fieldname === 'bonus_temp_company_logo')
                    {
                ...
                else
                {
                    ...
                    var outStr = fs.createWriteStream('storage/company/'+ document.corporate_id +'/cold_storage/bonus_temp_company_logo/bonus_temp_company_logo-' + Date.now()+'.png');
                    inStr.pipe(outStr);
                    document['company_logo']=outStr.path;
                }
                BonusSlipTemp.create(document,  function (err, bonus_slip_temp) {
                    if (err) return resp.status(200).send({ status: 'error', message: err.message });
                    return resp.status(200).send({ status: 'success',message:"Bonus slip template created successfully", bonus_slip_temp: bonus_slip_temp });
                })
            }
        }
        catch (e) {
    ...
    update_bonus_slip_temp: async function (req, resp) {
        try{
            const v = new Validator(req.body, {
                template_name:"required",
                bonus_slip_temp_id:"required",
                company_info: "required",
                employee_details: "required",
                bonus_details: "required",
                other_payment: "required",
            ...
            if (!matched) {
                ...
                var bonus_slip_temp_id=req.body.bonus_slip_temp_id;
                BonusSlipTemp.updateOne({'_id':bonus_slip_temp_id},document,  function (err, bonus_slip_temp) {
                    if (err) return resp.status(200).send({ status: 'error', message: err.message });
                    return resp.status(200).send({ status: 'success',message:"Bonus slip template updated successfully.", bonus_slip_temp: bonus_slip_temp });
                })
            }
        }
        catch (e) {
    ...
    update_publish_status: async function (req, resp) {
        ...
        if (!matched) {
            ...
            BonusSlipTemp.updateOne({'_id':req.body.template_id},document,  function (err, template_rule) {
                if (err) 
                {
    ...
    delete_bonus_slip_temp:async function (req, resp) {
        try{
            const v = new Validator(req.body, {
                bonus_slip_temp_id:'required',
            });
            ...
            if (!matched) {
                ...
                BonusSlipTemp.findByIdAndRemove({'_id':req.body.bonus_slip_temp_id},  function (err, bonus_slip_temp) {
                    if (err) 
                    {
                        return resp.status(200).send({ status: 'error', message: err.message });
                    }
                    else{
                        var filePath = file_path+'/'+bonus_slip_temp.company_logo;
                        if (fs.existsSync(filePath)) {
                            fs.unlinkSync(filePath);
                        } 
                        return resp.status(200).send({ status: 'success',message:"Bonus slip template deleted successfully", bonus_slip_temp: bonus_slip_temp });
                    }
